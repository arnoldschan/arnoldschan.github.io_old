...
#             recipients = [str(recipients)]
    ...
    def find_user(self,username):
        if self.followers=='':
            self.followers = api.getSelfUserFollowers()
        returning=None
    ...
    def direct_message_by_username(self,msg_text,username,media_id=''):
        self.uuid = self.generateUUID(True)
        if self.followers=='':
            self.followers=self.getTotalFollowers(self.username_id)
        msgto_id=self.find_user()
        if media_id == 'latest':
            self.getSelfUserFeed()
            media_id= api.LastJson['items'][0]['id']
        if media_id == '':
            response =self.direct_message(msg_text,msgto_id)
        else:
            response =self.direct_share(media_id,msgto_id,text=msg_text)
        if response:
